...
def resize_return_buffer(buf_, size_):
    """callback function that resize return buffer when it is too small
    Args:
...
class G2AnonModule(object):
    ...
    def init(self):
        ...
        retval = self._lib_handle.G2Anonymizer_init(
            self._module_name.encode("utf-8"),
        ...
        if retval == -2:
            self._lib_handle.G2Anonymizer_getLastException(
                self._return_buffer, self._return_buffer_size
            )
            raise TranslateG2ModuleException(self._return_buffer.value)
        elif retval == -1:
            raise G2ModuleNotInitialized(
                "G2AnonModule has not been successfully initialized"
            )
        elif retval < 0:
    ...
    def exportTokenLibrary(self):
        ...
        ret_code = self._lib_handle.G2Anonymizer_exportTokenLibrary(
            self._return_buffer, self._return_buffer_size, self._resize_func
        )
        if ret_code == -2:
            self._lib_handle.G2Anonymizer_getLastException(
                self._return_buffer, self._return_buffer_size
    ...
    def anonymize(self, record):
        ...
        ret_code = self._lib_handle.G2Anonymizer_anonymize(
            _record, self._return_buffer, self._return_buffer_size, self._resize_func
        )
        if ret_code == -2:
            self._lib_handle.G2Anonymizer_getLastException(
                self._return_buffer, self._return_buffer_size
